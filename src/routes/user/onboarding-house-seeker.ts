...
router.post("/api/users/onboarding-house-seeker", currentUser, requireAuth, [
    ...
    body('date_of_birth').trim().notEmpty().withMessage('what is your date of birth'),
    body('gender').trim().notEmpty().withMessage('what gender are you?'),
    body('occupation').trim().notEmpty().withMessage('what do you do?'),
    body('descriptions').isArray({ min: 1 }).withMessage('how do you describe yourself?'),
    body('interests').isArray({ min: 1 }).withMessage('please choose at least one interest'),
    ...
    body('cleans_room').trim().notEmpty().withMessage('how often do you clean your room?'),
    body('cooks').trim().notEmpty().withMessage('how often do you clean your room?'),
    body('partying').trim().notEmpty().withMessage('how often do you clean your room?'),
    body('religion').trim().notEmpty().withMessage('how often do you clean your room?'),
    ...
    const house_seeker = HouseSeeker.build({
        ...
        drinks,
        cleans_room,
        profile_image: profile_image ? profile_image : gender === "female" ? "https://res.cloudinary.com/diils/image/upload/v1677774464/wegeda/user_female_umai03.png" : "https://res.cloudinary.com/diils/image/upload/v1677774465/wegeda/user_male_wpb9rn.png",
        gallery_images,
        company,
        pets,
        church,
